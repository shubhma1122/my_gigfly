...
<header @class(['w-full z-10 fixed', 'main-header-scrolling' => !is_hero_section()]) id="main-header">
    <div x-data="window.TTRjRvxLbHzaKxW">
                ...
                <div class="flex items-center mx-auto">
                    <div class="flex items-center text-sm font-normal text-gray-500 dark:text-zinc-300 gap-x-2">
                        ...
                        <div>
                            ...
                            {{-- Text --}}
                            <span>{{ settings('general')->header_announce_text }}</span>

                            {{-- Link --}}
                            @if (settings('general')->header_announce_link)
                                <a href="{{ settings('general')->header_announce_link }}" target="_blank" class="inline font-medium text-primary-600 underline dark:text-primary-500 underline-offset-2 decoration-600 dark:decoration-500 decoration-solid hover:no-underline">
                                    @lang('messages.t_learn_more')
                                </a>
                            @endif
        ...
        <nav class="relative container max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 justify-between items-center h-20 flex">
                    ...
                    <div class="border dark:border-zinc-700 rounded-full flex items-center w-full py-0.5 px-6 mx-4" id="search-input-container">
                        ...
                        <div id="categories-menu-primary" class="group" x-data="{ selected_category: $el.dataset.category }" data-category="">
                            ...
                            <button type="button" class="relative flex items-center space-x-1 rtl:space-x-reverse bg-transparent text-xs+ font-semibold text-gray-500 focus:outline-none focus:ring-0 ltr:border-r ltr:pr-4 rtl:border-l rtl:pl-4 border-gray-200 dark:border-zinc-700 h-full dark:text-zinc-300">
                                <span>@lang('messages.t_categories')</span>
                            ...
                            <div class="absolute top-8 right-0 left-0 z-50 pt-14 invisible group-hover:visible max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
                                <div class="bg-white dark:bg-zinc-800 shadow-xl ring-1 ring-black ring-opacity-5 rounded-lg transform origin-top transition duration-300 ease-out opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 h-[calc(100vh-100px)] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-50">
                                  <div class="grid grid-cols-5 transform transition duration-500 ease-out h-full opacity-0 group-hover:opacity-100">
                                    ...
                                    <div class="p-8 space-y-6 bg-gray-50 h-full dark:bg-zinc-900">
                                        ...
                                        <nav class="flex flex-col space-y-3">
                                            @foreach ($categories as $category)
                                                ...
                                                {{-- First category in loop --}}
                                                @if ($loop->first)
                                                    <script>
                                                        let primary_category_menu = document.getElementById("categories-menu-primary"); 
                                                        primary_category_menu.setAttribute("data-category", "{{ $category->id }}"); 
                                                    </script>
                                                @endif

                                                {{-- Parent category --}}
                                                <a href="{{ url('categories', $category->slug) }}" class="text-gray-500 dark:text-zinc-300 hover:text-primary-600 dark:hover:text-primary-600 font-semibold text-xs+ ltr:text-left rtl:text-right" wire:key="header-categories-button-{{ $category->uid }}" x-on:mouseover="selected_category = '{{ $category->id }}'">
                                                    {{ $category->name }}
                                                </a>
                                    ...
                                    @foreach ($categories as $category)
                                        <div class="col-span-4 p-8 space-y-6 bg-white dark:bg-zinc-800" wire:key="header-categories-container-{{ $category->uid }}" x-show="selected_category === '{{ $category->id }}'" x-cloak>

                                            {{-- Subcategories and childcategories --}}
                                            <div class="grid grid-cols-4 gap-4 xl:gap-8">
                                                ...
                                                @foreach ($category->subcategories as $subcategory)
                                                    <div class="block space-y-3">
                                                        ...
                                                        {{-- Subcategory --}}
                                                        <a href="{{ url('categories/' . $category->slug . '/' . $subcategory->slug) }}" class="text-sm font-bold text-gray-900 dark:text-zinc-100 hover:underline">
                                                            {{ $subcategory->name }}
                                                        </a>
                                                        ...
                                                        <ul class="space-y-1">
                                                            @foreach ($subcategory->childcategories as $childcategory)
                                                                <li>
                                                                    <a href="{{ url('categories/' . $category->slug . '/' . $subcategory->slug . '/' . $childcategory->slug) }}" class="text-gray-500 dark:text-zinc-300 dark:hover:text-primary-600 hover:text-primary-600 font-semibold text-xs+ ltr:text-left rtl:text-right flex items-center space-x-2 rtl:space-x-reverse">
                                                                        @if ($childcategory->icon)
                                                                            <img src="{{ src($childcategory->icon) }}" alt="{{ $childcategory->name }}" class="w-5 h-5">
                                                                        @endif
            ...
            <div class="min-w-fit ltr:sm:ml-10 rtl:sm:mr-10 flex items-center font-medium text-sm transition-all duration-200">
                ...
                {{-- Messages --}}
                @auth
                    <a href="{{ url('inbox') }}" class="text-gray-500 hover:text-primary-600 transition-colors duration-300 py-2 relative mx-4 dark:text-gray-100 dark:hover:text-white hidden md:block">
                ...
                <div class="items-center space-x-7 rtl:space-x-reverse ltr:ml-5 rtl:mr-5 hidden lg:flex">
                            ...
                            <div class="bg-white dark:bg-zinc-800 shadow-xl ring-1 ring-black ring-opacity-5 rounded-lg transform origin-top transition duration-300 ease-out opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 overflow-hidden">
                                <div class="p-3 space-y-6 transform transition duration-500 ease-out opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100">
                                    <nav class="flex flex-col space-y-1">
                                        ...
                                        {{-- Gigs --}}
                                        <a href="{{ url('search') }}" class="p-3 rounded-xl flex items-start space-x-3 rtl:space-x-reverse text-gray-600 dark:text-zinc-200 font-medium text-xs+ hover:bg-slate-50 dark:hover:bg-zinc-700">
                                            <div class="flex-none flex items-center justify-center">
                                                <i class="ph-duotone ph-images-square text-2xl text-gray-400 dark:text-gray-300"></i>
                                            </div>
                                            <div class="space-y-0.5">
                                                <h5 class="font-bold">@lang('messages.t_explore_gigs')</h5>
                                                <p class="opacity-75 text-xs">
                                                    @lang('messages.t_find_the_right_service_for_every_price_point')
                                                </p>
                                            </div>
                                        </a>
                                        ...
                                        @if (settings('projects')->is_enabled)
                                            <a href="{{ url('explore/projects') }}" class="p-3 rounded-xl flex items-start space-x-3 rtl:space-x-reverse text-gray-600 dark:text-zinc-200 font-medium text-xs+ hover:bg-slate-50 dark:hover:bg-zinc-700">
                                                <div class="flex-none flex items-center justify-center">
                                                    <i class="ph-duotone ph-briefcase text-2xl text-gray-400 dark:text-gray-300"></i>
                                                </div>
                                                <div class="space-y-0.5">
                                                    <h5 class="font-bold">@lang('messages.t_explore_projects')</h5>
                                                    <p class="opacity-75 text-xs">
                                                        @lang('messages.t_hire_the_best_freelancers_for_any_job_online')
                                                    </p>
                                                </div>
                                            </a>
                                        @endif
                                        ...
                                        @if (settings('blog')->enable_blog)
                                            <a href="{{ url('blog') }}" class="p-3 rounded-xl flex items-start space-x-3 rtl:space-x-reverse text-gray-600 dark:text-zinc-200 font-medium text-xs+ hover:bg-slate-50 dark:hover:bg-zinc-700">
                                                <div class="flex-none flex items-center justify-center">
                                                    <i class="ph-duotone ph-newspaper text-2xl text-gray-400 dark:text-gray-300"></i>
                                                </div>
                                                <div class="space-y-0.5">
                                                    <h5 class="font-bold">@lang('messages.t_blog')</h5>
                                                    <p class="opacity-75 text-xs">
                                                        @lang('messages.t_news_information_and_community_stories')
                                                    </p>
                                                </div>
                                            </a>
                                        @endif
                                        ...
                                        {{-- Support center --}}
                                        <a href="{{ url('help/contact') }}" class="p-3 rounded-xl flex items-start space-x-3 rtl:space-x-reverse text-gray-600 dark:text-zinc-200 font-medium text-xs+ hover:bg-slate-50 dark:hover:bg-zinc-700">
                                            <div class="flex-none flex items-center justify-center">
                                                <i class="ph-duotone ph-chats text-2xl text-gray-400 dark:text-gray-300"></i>
                                            </div>
                                            <div class="space-y-0.5">
                                                <h5 class="font-bold">@lang('messages.t_help_and_support')</h5>
                                                <p class="opacity-75 text-xs">
                                                    @lang('messages.t_how__can_we_help_u')
                                                </p>
                                            </div>
                                        </a>
                    ...
                    @auth
                        <div class="relative group">
                            ...
                            <button class="main-header-link-item leading-5 flex items-center text-xs+ font-semibold space-x-2 rtl:space-x-reverse" type="button">
                                ...
                                {{-- Avatar --}}
                                @if (auth()->user()->avatar)
                                    <img class="object-cover w-8 h-8 rounded-full" src="{{ src(auth()->user()->avatar) }}" alt="{{ auth()->user()->username }}">
                                @else
                                ...
                                {{-- Username --}}
                                <span class="dark:text-zinc-300 dark:hover:text-zinc-100">{{ auth()->user()->username }}</span>
                                        ...
                                        <div class="divide-y divide-slate-100 dark:divide-zinc-700">
                                            ...
                                            <div class="py-1.5 px-3.5">
                                                ...
                                                @if (auth()->user()->account_type === 'buyer')
                                                    ...
                                                    <a href="{{ url('start_selling') }}"
                                                        ...
                                                        <span
                                                            class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_become_a_seller') }}</span>
                                                    </a>
                                                @endif
                                                ...
                                                @if (auth()->user()->account_type === 'seller')
                                                    ...
                                                    <a href="{{ url('seller/home') }}"
                                                        ...
                                                        <span
                                                            class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_seller_dashboard') }}</span>
                                                    </a>
                                                @endif
                                            ...
                                            @if (settings('projects')->is_enabled)
                                                ...
                                                <div class="py-1.5 px-3.5">
                                                    ...
                                                    {{-- My projects --}}
                                                    <a href="{{ url('account/projects') }}" class="group flex items-center py-1.5 hover:text-primary-600">
                                                    ...
                                                    @if (settings('projects')->who_can_post === 'both' || settings('projects')->who_can_post === auth()->user()->account_type)
                                                        <a href="{{ url('post/project') }}" class="group flex items-center py-1.5 hover:text-primary-600">
                                            ...
                                            @if (settings('publish')->enable_custom_offers)
                                                <div class="py-1.5 px-3.5">
                                                    <a href="{{ url('account/offers') }}" class="group flex items-center py-1.5 hover:text-primary-600">
                                            ...
                                            <div class="py-1.5 px-3.5">
                                                ...
                                                <a href="{{ url('profile', auth()->user()->username) }}"
                                                    ...
                                                    <span
                                                        class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_view_profile') }}</span>
                                                </a>
                                                ...
                                                <a href="{{ url('account/profile') }}"
                                                    ...
                                                    <span
                                                        class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_edit_profile') }}</span>
                                                </a>
                                                ...
                                                <a href="{{ url('account/settings') }}"
                                                    ...
                                                    <span
                                                        class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_account_settings') }}</span>
                                                </a>
                                                ...
                                                <a href="{{ url('account/password') }}"
                                                    ...
                                                    <span class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_update_password') }}</span>
                                                </a>
                                            ...
                                            <div class="py-1.5 px-3.5">
                                                ...
                                                {{-- Deposit --}}
                                                <a href="{{ url('account/deposit') }}" class="group flex items-center py-1.5 hover:text-primary-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="flex-none ltr:mr-3 rtl:ml-3 text-gray-400 hover:text-primary-600 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                                                    <span class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_deposit') }}</span>
                                                </a>
                                                ...
                                                <a href="{{ url('account/orders') }}"
                                                    ...
                                                    <span
                                                        class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_my_orders') }}</span>
                                                </a>
                                                ...
                                                <a href="{{ url('inbox') }}"
                                                    ...
                                                    <span
                                                        class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_messages') }}</span>
                                                </a>
            
                                                {{-- Reviews --}}
                                                <a href="{{ url('account/reviews') }}" class="group flex items-center py-1.5 hover:text-primary-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="flex-none ltr:mr-3 rtl:ml-3 text-gray-400 hover:text-primary-600 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                                                    <span class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_reviews') }}</span>
                                                </a>
            
                                                {{-- Refunds --}}
                                                <a href="{{ url('account/refunds') }}" class="group flex items-center py-1.5 hover:text-primary-600">
                                                    ...
                                                    <span class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_refunds') }}</span>
                                                </a>
                                            ...
                                            <div class="py-1.5 px-3.5">
                                                ...
                                                @if (auth()->user()->status !== 'verified')
                                                    <a href="{{ url('account/verification') }}"
                                                        ...
                                                        <span
                                                            class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_verification_center') }}</span>
                                                    </a>
                                                @endif
                                                ...
                                                <a href="{{ url('auth/logout') }}"
                                                    ...
                                                    <span
                                                        class="font-semibold text-xs text-gray-700 dark:text-gray-100 hover:text-primary-600 dark:hover:text-primary-500">{{ __('messages.t_logout') }}</span>
                                                </a>
                    ...
                    {{-- Divider --}}
                    @if (auth()->guest() || ( auth()->check() && auth()->user()->account_type === 'buyer' ))
                        <span class="inline mx-5 w-px h-5 main-header-divider dark:bg-zinc-800"></span>
                    @endif
    
                    {{-- Register / Find Freelancers --}}
                    @if (auth()->guest() || ( auth()->check() && auth()->user()->account_type === 'buyer' ))
                        <div class="space-x-3 rtl:space-x-reverse">
            
                            {{-- Sign up --}}
                            @guest
                                <a href="{{ url('auth/register') }}" class="bg-[#f8f7f4] hover:bg-zinc-100 leading-5 font-semibold px-4 pt-2 pb-[9px] rounded-3xl text-xs+ text-zinc-500">
                                    @lang('messages.t_signup')
                                </a>
                            @endguest

                            {{-- Find freelancers --}}
                            @if (auth()->guest() || ( auth()->check() && auth()->user()->account_type === 'buyer' ))
                                <a href="{{ url('sellers') }}" class="bg-[#0d0c22] dark:bg-zinc-800 dark:hover:bg-zinc-700 hover:bg-[#565564] leading-5 font-semibold px-4 pt-2 pb-[9px] rounded-3xl text-xs+ text-white ltr:ml-2 rtl:mr-2">
