...
function uid($length = 20)
{
    ...
    return mb_strtoupper(substr($random, 0, $length));
}
...
function getWebsiteFirstLetter($domain)
{
    try {
        ...
        // Get first letter
        $firstLetter   = mb_strtoupper(substr($withoutDomain[0], 0, 1));
...
{
    return asset('vendor/blade-flags/country-' . strtolower($code) . '.svg');
}
...
function generate_jazzcash_hash($payload)
{
    ...
    return mb_strtoupper(hash_hmac('sha256', $hashString, $jazzcash_salt));
}
...
/**
 * Get sweetalert params
 *
 * @param string $message
 * @param string $type
 * @return array
 */
function livewire_alert_params($message, $type = "success")
{
    // Check message type
    switch ($type) {
        ...
        case 'success':
            ...
            return [
                'text'             => $message,
                'iconHtml'         => config('livewire-alert.icons.success'),
                'cancelButtonText' => config('livewire-alert.icons.cancel_btn'),
                'customClass'      => [
                    'popup'             => 'max-w-sm w-full bg-white !shadow-lg rounded-lg ring-1 ring-black
                    ring-opacity-5 relative overflow-hidden pointer-events-auto
                    dark:bg-zinc-800 dark:border dark:border-zinc-700',
                    'htmlContainer'     => '!text-xs !text-gray-400',
                    'actions'           => '!hidden',
                    'cancelButton'      => 'inline-flex rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'
                ]
            ];
        ...
        case 'error':
            ...
            return [
                'text'             => $message,
                'iconHtml'         => config('livewire-alert.icons.error'),
                'cancelButtonText' => config('livewire-alert.icons.cancel_btn'),
                'customClass'      => [
                    'popup'             => 'max-w-sm w-full bg-white !shadow-lg rounded-lg ring-1 ring-black
                    ring-opacity-5 relative overflow-hidden pointer-events-auto
...
function is_hero_section() : bool
{
    try {
        ...
        // Hero section will be enabled only for main route
...
function getTextIndentValue() : int
{
    ...
    // Get default lang
    $default_lang = app()->getLocale() ?? $settings?->default_language;

    // Get language
    $language     = Language::where('language_code', $default_lang)->select('params')->first();
