            ...
            if ( $invoice_id && $order_id ) {
                ...
                if ( is_array($response) && $response['success'] === TRUE) {
                    ...
                    if (Str::startsWith($order_id, "DD")) {
                        ...
                        $data = DepositWebhook::where('payment_id', $invoice_id)
                    ...
                    if (Str::startsWith($order_id, "GG")) {
                        ...
                        // Get saved webhook data in our database
                        $data = CheckoutWebhook::where('payment_id', $invoice_id)
                                                ->where('payment_method', $this->gateway)
                                                ->where('status', 'pending')
                                                ->first();
                    ...
                    if (Str::startsWith($order_id, "PP")) {
                        ...
                        // Get subscription
                        $subscription = ProjectSubscription::where('payment_id', $invoice_id)
                                                            ->where('status', 'pending')
                                                            ->where('payment_method', $this->gateway)
                                                            ->with('project')
                                                            ->first();
                    ...
                    if (Str::startsWith($order_id, "BB")) {
                        ...
                        // Get subscription
                        $subscription = ProjectBidUpgrade::where('payment_id', $invoice_id)
                                            ->where('status', 'pending')
                                            ->where('payment_method', $this->gateway)
                                            ->with('bid')
                                            ->first();
