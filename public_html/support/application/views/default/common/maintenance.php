...
<?php if ( db_config( 'i_pc_status' ) != 3 ) { ?>
  <title><?php echo  manage_title( lang( 'under_maintenance' ) ); ?></title>
<?php } else { ?>
  <title><?php echo manage_title( err_lang( 'went_wrong' ) ); ?></title>
<?php } ?>
...
<div class="z-align-middle">
  <div class="card z-card-mini-box text-center">
    ...
    <?php } else { ?>
      <img class="mb-4" src="<?php illustration_by_color( 'not_found' ); ?>" alt="">
      <p class="mb-0 fw-bold text-white bg-danger rounded small"><?php echo err_lang( 'went_wrong' ); ?></p>
    <?php } ?>
  </div>
  <!-- /.card -->
</div>
