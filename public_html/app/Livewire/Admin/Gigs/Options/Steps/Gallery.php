    ...
    public function addVideo()
    {
        ...
        if (isset($matches[1])) {
            ...
            $headers = get_headers('https://www.youtube.com/oembed?format=json&url=http://www.youtube.com/watch?v=' . $matches[1]);
                ...
                if(is_array($headers) ? preg_match('/^HTTP\\/\\d+\\.\\d+\\s+2\\d\\d\\s+.*$/',$headers[0]) : false){
                    ...
                    $this->alert(
                        ...
                        livewire_alert_params( __('messages.t_youtube_video_has_been_successfully_added') )
                    );

                    // Close modal
                    $this->dispatch('close-modal', 'modal-add-youtube-video-container');
                    ...
                    $this->alert(
                        'error', 
                        __('messages.t_error'), 
                        livewire_alert_params( __('messages.t_youtube_video_not_exists'), 'error' )
                    );
            ...
            $this->alert(
                'error', 
                __('messages.t_error'), 
                livewire_alert_params( __('messages.t_youtube_video_invalid'), 'error' )
            );
