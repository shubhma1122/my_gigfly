                        ...
                        <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16" wire:key="admin-dashboard-subscriptions-{{ $s->uid }}">
                            ...
                            <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                <div class="flex items-center justify-center space-x-1 rtl:space-x-reverse">
                                    @if($s->payment_method == 'offline')
                                              
                                        @if (!empty(payment_gateway('offline', false, true)->name))
                                            <span class="font-semibold text-sm text-gray-700">
                                                {{ payment_gateway("offline", false, true)->name }}
                                            </span>
                                        @else
                                            <span class="font-semibold text-sm text-gray-700">
                                                {{ __('messages.t_offline_payment') }}
                                            </span>
                                        @endif

                                    @elseif($s->payment_method == 'wallet')
                                        <span class="font-semibold text-sm text-gray-700">
                                            {{ __('messages.t_wallet') }}
                                        </span>
                                    @elseif( !empty(payment_gateway($s->payment_method)?->name) )

                                        <span class="font-semibold text-sm text-gray-700">
                                            {{ payment_gateway($s->payment_method)->name }}
                                        </span>
                            ...
                            <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                ...
                                @elseif ($s->status === 'pending' && $s->payment_method === 'offline')
