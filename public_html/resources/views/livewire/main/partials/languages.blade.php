<div class="block relative">
    ...
    <button type="button" onclick="showModal('main-languages-modal')" class="bg-zinc-100 border border-zinc-200 dark:border-transparent dark:bg-zinc-700 duration-200 ease-linear flex h-8 hover:bg-opacity-90 items-center justify-center rounded-full transition-colors w-8 hover:bg-zinc-200 dark:hover:bg-zinc-600">
        <img class="w-4.5 h-4.5" src="{{ countryFlag($default_country_code) }}" alt="{{ $default_language_name }}">
    </button>

    {{-- Modal --}}
    @push('scripts')
        <x-bladewind.modal size="tiny" ok_button_label="" name="main-languages-modal">
                ...
                <div class="space-y-3">
                    @foreach (supported_languages() as $lang)
                        ...
                        <a href="{{ $locale_link }}" rel="nofollow" class="py-2 px-4 rounded-sm inline-flex items-center cursor-pointer justify-between {{ $default_language_code === $lang->language_code ? 'bg-primary-100/25 text-primary-600' : 'hover:bg-gray-50 dark:hover:bg-zinc-600' }} focus:outline-none w-full">
