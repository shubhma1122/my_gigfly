								...
								<div>
									<button 
                                        ...
                                        <div class="h-8 w-8 inline-flex flex-shrink-0 relative">
                                            @if (auth()->user()->avatar)
                                                <img class="mask is-squircle object-cover w-full h-full block" src="{{ src(auth()->user()->avatar) }}" alt="{{ auth()->user()->username }}">
                                            @else
                                                @php
                                                    $faker = Faker\Factory::create();
                                                    $color = $faker->rgbColor();
                                                @endphp
                                                <div class="flex items-center justify-center h-full w-full mask is-squircle text-sm uppercase font-medium" style="background-color: rgba({{ $color }}, .1);color: rgb({{ $color }})">
