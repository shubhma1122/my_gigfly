...
<div class="relative px-2 py-3 border-2 border-dashed border-gray-300 dark:text-dark-300 dark:border-zinc-600
    dark:bg-dark-800 hover:dark:border-gray-400 text-center cursor-pointer rounded-md bw-fp-{{ $name }} @if($add_clearing) mb-3 @endif">
    ...
    <x-bladewind.icon name="x-circle" class="absolute ltr:right-3 rtl:left-3 h-6 w-6 text-gray-400 hover:text-red-600 clear cursor-pointer hidden" type="solid" />
...
@push('scripts')
    <script>
        ...
        dom_el('.bw-fp-{{ $name }}').addEventListener('drop', function (evt){
            changeCss('.bw-fp-{{ $name }}','border-gray-400', 'remove');
        ...
        ['dragenter', 'dragover', 'mouseover'].forEach(evt =>
            dom_el('.bw-fp-{{ $name }}').addEventListener(evt, () => {
                changeCss('.bw-fp-{{ $name }}','border-gray-400');
            }, false)
        );
